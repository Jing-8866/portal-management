...
public class OrderController {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("@subsystemAuth.hasLogin('ORDER_MGMT')")
    @OperationLog(value = "删除订单", subsystem = "ORDER_MGMT")
    public ApiResult<Boolean> deleteOrder(@PathVariable Long id) {
        try {
            return ApiResult.success(orderService.deleteOrder(id));
        } catch (RuntimeException e) {
            return ApiResult.error(e.getMessage());
        }
    }
